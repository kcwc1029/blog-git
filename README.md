...
-   [成大工科 工程科學暨創新概論 git 教學 01](https://www.youtube.com/watch?v=xCbN6xqMq4w)
    -   Git 基礎概念
    -   集中式版本控制 (CVCS) vs. 分散式版本控制 (DVCS)
    -   Git 和 GitHub 的區別
    -   Git 安裝與初始化
    -   Git 三種狀態
    -   PC 與雲端之間連結操作
    -   .gitignore 的作用與寫法
    -   Git Tag（版本標記）
-   [成大工科 工程科學暨創新概論 git 教學 02](https://www.youtube.com/watch?v=2nl6M9ar494)
    -   版本切換
    -   Detached HEAD
    -   Git Flow（分支開發流程）
    -   分支管理（branch）
...
## 4. 集中式版本控制 (Centralized Version Control, CVCS) vs. 分散式版本控制 (Distributed Version Control, DVCS)

### 4.1. 集中式版本控制 (Centralized Version Control, CVCS)
...
### 4.2. 分散式版本控制 (Distributed Version Control, DVCS) => git
...
### 8.1. Lab：[github 建立 Repository，clone 到 PC 使用](./Lab/github_to_PC.md)
...
### 11.1. Lab：在 github 上修改 README，在 PC 端 pull

-   在 github 上修改 README：增加文字【這是直接在 github 上新增的文字】
-   在 PC 上 git pull
-   在 PC 上對 README 新增文字【這是直接在 PC 上新增的文字】

## 12. `.gitignore`

### 12.1. Lab：建立檔案 key.txt，並利用 gitignore 忽略它
...
### 13.1. Lab：將檔案 key.txt commit ，在利用 gitignore 跟 rm 清除她

-   嘗試將 gitignore 的文字註解調，讓私人檔案做紀錄
-   在利用 gitignore 跟 rm 清除她削掉 PC 紀錄
...
-   返回之前 commit 的時機
    -   只是想回去看看
    -   回前面看看，並加給行註記(跟現階段完全無關) => 做標記之類 => 不會影響到現今開發的分支
...
### 15.2. git checkout 和 Detached HEAD
...
### 15.4. Lab：在 Detached HEAD 狀態下做了更改，並希望保存這些更改
...
### 15.5. 重置當前分支(HEAD)的狀態：git reset
